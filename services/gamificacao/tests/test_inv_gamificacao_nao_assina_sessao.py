"""Teste-guarda [INV-P12]: a célula `gamificacao` NUNCA assina o cookie de sessão.
...
produzem um cabo-de-guerra silencioso — abrir a página de conquistas desloga do
site, entrar no site desloga daqui — **sem erro em lugar nenhum, sem log, sem
...
**E nesta célula a tentação é maior do que em qualquer outra, e tem nome: a
CELEBRAÇÃO VISCERAL.** O desenho manda a comemoração aparecer no segundo em que
o marco é validado, em tela cheia, uma vez só (`VEREDITO.md` §3.8; auditoria
5.3). Toda tela assim precisa responder "esta pessoa já viu esta comemoração?"
— e o caminho de menor esforço para guardar essa lembrança é
`request.session[...]`. Funciona em dev, passa em teste de unidade, e só quebra
em produção, onde existe um segundo assinante do mesmo cookie. É por isso que o
plano põe `celebracoes_pendentes` no MODELO
(`PLANO-CELULA-GAMIFICACAO.md` §3) e não na sessão: o estado é do aluno, e o
cookie do site não é lugar de estado de ninguém além da `identidade`.
...
def test_sem_middleware_de_sessao():
    ...
    assert not any("SessionMiddleware" in m for m in settings.MIDDLEWARE), (
        "a célula gamificacao não pode ter SessionMiddleware — quem assina "
...
def test_sem_app_de_sessao():
    ...
    ), "a célula gamificacao não pode instalar django.contrib.sessions"
...
def test_sem_backend_de_sessao_declarado_pela_celula():
    ...
    assert not hasattr(settings_da_celula, "SESSION_ENGINE"), (
        "a célula gamificacao não declara SESSION_ENGINE — quem assina sessão "
...
def test_o_cookie_de_csrf_tem_nome_proprio():
    ...
    administrativa em `/admin`, o fórum em `/forum` e esta célula em
    `/conquistas`. Duas células gravando `csrftoken` no mesmo host é uma
    invalidando o formulário da outra.
    """
    assert settings.CSRF_COOKIE_NAME == "gamificacao_csrf"
