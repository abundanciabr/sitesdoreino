...
1. **O urlconf não pode conhecer o prefixo.** Quem o aplica é
   ...
   como `path("conquistas/healthz", ...)` deixa de resolver assim que o prefixo
...
2. **Qualquer isenção de middleware compara `request.path_info`, nunca
   ...
   request line que chega ao uvicorn é `GET /conquistas/healthz`, e nessa
   requisição `request.path` contém o prefixo em QUALQUER versão do Django.
   `request.path_info` segue `/healthz` nos dois casos.

Esta célula ainda não tem middleware próprio — a porta nasce com a primeira
tela (PR 7 da escada). **O guarda é plantado ANTES de propósito**: quem
...
`/conquistas/<caminho>` pela internet. Vale para o `/healthz` (o que é
desejado) e valerá para o `/interno/` da porta de máquina do PR 16 (o que
exige Bearer, não topologia). O `test_o_prefixo_alcanca_a_raiz_do_urlconf`
...
|---------------------|------------------------|-------------------------------|
| borda pública       | `/conquistas/healthz`  | ASGI (uvicorn atrás do Traefik) |
...
# O prefixo público real da célula (`infra/env/gamificacao.env`, PR 5 da
...
def env_de_producao(settings):
    """O que o env real da VPS faz: SCRIPT_NAME=/conquistas."""
...
def test_o_prefixo_alcanca_a_raiz_do_urlconf(env_de_producao):
    ...
    urlconf é servida em `/conquistas/<rota>` pela internet. É por isso que o
    `/healthz` funciona pela borda — e é exatamente por isso que a porta de
    máquina do PR 16 (`/interno/...`) NÃO poderá contar com a topologia para
    ficar escondida: ela nascerá publicada, e quem a fecha é o Bearer do par.
...
def test_urlconf_nao_conhece_o_prefixo(client):
    ...
    Sem SCRIPT_NAME configurado, `/conquistas/healthz` NÃO é a sonda — se este
    teste virar 200 com o JSON de saúde, alguém embutiu `/conquistas` numa rota
