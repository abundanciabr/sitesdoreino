...
#
# Cópia do PADRÃO de `alunos` (Lei 3: copia-se o padrão entre células, nunca se
# importa código de uma na outra). As quatro funções de limpeza abaixo existem
# porque o django-ninja emite ruído que o contrato escrito à mão não tem — e o
# freeze compara os dois byte a byte, então "ruído cosmético" reprova o CI
# exatamente como uma divergência real.
#
# Este comando é o que torna `contracts/sugestoes.openapi.yaml` um PORTÃO em vez
# de documentação: sem ele, o manifesto não teria como medir drift e a entrada
# desta célula no freeze seria promessa sem mecanismo (RETROSPECTIVA §2).
import json
