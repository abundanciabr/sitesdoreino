# apps/matriculas/tasks.py
"""O relay da outbox: as cartas desta célula saem daqui para o Redis Streams.

Espelha o relay que já roda em produção na `sugestoes` (e antes dela em
`pagamentos`, `quiz` e `checkout`). Não é falta de imaginação: um relay
diferente por célula significaria N modos de falha diferentes para o mesmo
...
def relay_apos_commit() -> None:
    ...
    nem quebra a decisão do mantenedor: ele segue na outbox com
    `published_at=None`, e a task periódica abaixo republica. Por isso o
    `except` largo — é defensivo por desenho, não descuido.
    """
    try:
        relay_outbox()
...
def relay_outbox_periodico() -> int:
    ...
    reclama (`armadilhas/030`). No compose ele é o serviço `alunos-relay`.
    """
    return relay_outbox()
