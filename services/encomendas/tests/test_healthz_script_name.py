...
1. **O urlconf não pode conhecer o prefixo.** Quem o aplica é
   ...
   como `path("encomendas/healthz", ...)` deixa de resolver assim que o prefixo
...
2. **Qualquer isenção de middleware compara `request.path_info`, nunca
   ...
   request line que chega ao uvicorn é `GET /encomendas/healthz`, e nessa
   requisição `request.path` contém o prefixo em QUALQUER versão do Django.
   `request.path_info` segue `/healthz` nos dois casos.

Esta célula ainda não tem middleware próprio — a porta nasce com o
reconhecimento de sessão (degrau 2.7 da escada). **O guarda é plantado ANTES
de propósito**: quem escrever a porta sobre `request.path` encontra este
arquivo vermelho no `make ci`, em vez de encontrar a sonda morta em produção e
o container nunca ficando `healthy`.
...
esta célula servir na raiz do urlconf fica alcançável em
`/encomendas/<caminho>` pela internet. Vale para o `/healthz` (o que é
desejado) e valerá para `/api/encomendas/` e `/interno/` da porta de máquina
(o que exige Bearer, não topologia). O `test_o_prefixo_alcanca_a_raiz_do_urlconf`
abaixo é a prova dessa premissa, escrita agora para que ninguém copie de uma
célula vizinha a frase *"nada em `/interno` resolve pela borda pública"* — que
aqui seria FALSA.
...
|---------------------|------------------------|-------------------------------|
| borda pública       | `/encomendas/healthz`  | ASGI (uvicorn atrás do Traefik) |
...
# O prefixo público real da célula (`infra/env/encomendas.env`, degrau 2.10 da
...
def env_de_producao(settings):
    """O que o env real da VPS faz: SCRIPT_NAME=/encomendas."""
...
def test_o_prefixo_alcanca_a_raiz_do_urlconf(env_de_producao):
    ...
    urlconf é servida em `/encomendas/<rota>` pela internet. É por isso que o
    `/healthz` funciona pela borda — e é exatamente por isso que a porta de
    máquina do degrau 2.7 (`/api/encomendas/`, `/interno/`) NÃO poderá contar
...
def test_urlconf_nao_conhece_o_prefixo(client):
    ...
    Sem SCRIPT_NAME configurado, `/encomendas/healthz` NÃO é a sonda — se este
    teste virar 200 com o JSON de saúde, alguém embutiu `/encomendas` numa rota
    e a célula deixou de ser dona do próprio prefixo por configuração.
