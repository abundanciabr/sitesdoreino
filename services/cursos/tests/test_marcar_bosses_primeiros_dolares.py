...
def test_marca_um_boss_por_modulo():
    ...
    for ordem, titulo in enumerate(TITULOS_REAIS_DOS_BOSSES, start=1):
...
@pytest.mark.django_db
def test_recusa_duas_aulas_com_o_titulo_do_boss_sem_mudar_bosses():
    curso = Curso.objects.create(
        site_id="escola-a",
        slug="primeiros-dolares",
        nome="Primeiros Dólares com Roblox",
    ...
    for ordem, titulo in enumerate(TITULOS_REAIS_DOS_BOSSES, start=1):
        ...
        if ordem == 3:
            Aula.objects.create(
                ...
                ordem=ordem * 10 + 1,
                numero=f"{ordem}B",
                titulo_exibido="COMANDO BEVEL E TRIANGULATE!",
            )

    with pytest.raises(CommandError, match="encontrei 2"):
        call_command("marcar_bosses_primeiros_dolares", stdout=StringIO())
