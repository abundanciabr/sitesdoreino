...
Esta célula serve em `meshcraft.top/cursos` (`PLANO-CELULA-CURSOS.md` §6), ou
seja **sob SCRIPT_NAME** — a mesma condição que derrubou a sonda do `checkout`
(PR #65) e do `quiz` (PR #71), e que a `sugestoes`, a `admin`, o `forum`, a
`gamificacao` e as `encomendas` já travam do mesmo jeito. Duas coisas quebram
nesse regime, e as duas estão travadas aqui:
...
1. **O urlconf não pode conhecer o prefixo.** Quem o aplica é
   ...
   como `path("cursos/healthz", ...)` deixa de resolver assim que o prefixo
...
2. **Qualquer isenção de middleware compara `request.path_info`, nunca
   ...
   request line que chega ao uvicorn é `GET /cursos/healthz`, e nessa
   requisição `request.path` contém o prefixo em QUALQUER versão do Django.
   `request.path_info` segue `/healthz` nos dois casos.

Esta célula ainda não tem middleware próprio — a porta nasce com a porta de
máquina (degrau 1.3 da escada). **O guarda é plantado ANTES de propósito**:
quem escrever a porta sobre `request.path` encontra este arquivo vermelho no
`make ci`, em vez de encontrar a sonda morta em produção e o container nunca
ficando `healthy`.
...
esta célula servir na raiz do urlconf fica alcançável em `/cursos/<caminho>`
pela internet. Vale para o `/healthz` (o que é desejado) e valerá para
`/api/cursos/` e `/interno/` da porta de máquina (o que exige Bearer, não
topologia). O `test_o_prefixo_alcanca_a_raiz_do_urlconf` abaixo é a prova
dessa premissa, escrita agora para que ninguém copie de uma célula vizinha a
frase *"nada em `/interno` resolve pela borda pública"* — que aqui seria FALSA.

Os dois caminhos de entrada são exercitados pelo transporte REAL de cada um:

| Caminho             | Request line       | Transporte                        |
|---------------------|--------------------|-----------------------------------|
| borda pública       | `/cursos/healthz`  | ASGI (uvicorn atrás do Traefik)   |
...
# O prefixo público real da célula (`infra/env/cursos.env`, degrau 1.7 da
# escada). Escrito à mão aqui, e não lido de `settings`: um teste que lê a
# mesma variável que o código passaria mesmo com o valor errado.
PREFIXO = "/cursos"


@pytest.fixture
def env_de_producao(settings):
    """O que o env real da VPS faz: SCRIPT_NAME=/cursos."""
...
def test_o_prefixo_alcanca_a_raiz_do_urlconf(env_de_producao):
    ...
    urlconf é servida em `/cursos/<rota>` pela internet. É por isso que o
    `/healthz` funciona pela borda — e é exatamente por isso que a porta de
    máquina do degrau 1.3 (`/api/cursos/`, `/interno/`) NÃO poderá contar
    com a topologia para ficar escondida: ela nascerá publicada, e quem a fecha
    é o Bearer do par.

    A medida é o `path_info` já cortado: se um dia o corte deixar de acontecer,
    esta asserção cai e o comentário do `config/urls.py` deixa de estar
    sozinho defendendo a afirmação.
    """
    resp = async_to_sync(AsyncClient().get)(f"{PREFIXO}/healthz")
    assert resp.asgi_request.path_info == "/healthz", (
...
def test_urlconf_nao_conhece_o_prefixo(client):
    ...
    Sem SCRIPT_NAME configurado, `/cursos/healthz` NÃO é a sonda — se este
    teste virar 200 com o JSON de saúde, alguém embutiu `/cursos` numa rota e
    a célula deixou de ser dona do próprio prefixo por configuração.

    A asserção é `!= 200` e não `== 404` de propósito: quando a porta nascer,
    esse caminho passará a responder um redirecionamento em vez de 404, e o
    que este guarda precisa provar continua sendo o mesmo — aquele caminho
    **não entrega a sonda**.
    """
