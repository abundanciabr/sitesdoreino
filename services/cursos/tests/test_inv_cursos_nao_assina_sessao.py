"""Teste-guarda [INV-P12]: a célula `cursos` NUNCA assina o cookie de sessão.
...
produzem um cabo-de-guerra silencioso — abrir a aula desloga do site, entrar
...
**E nesta célula a tentação tem dois nomes.** A CERIMÔNIA DO BOSS: quando a
última aula de um Bloco abre a porta, a tela cheia celebra, uma vez só, e toda
tela assim precisa responder "esta pessoa já viu?". E o LAUDO RECEBIDO: "o
aluno já leu a devolução?" decide se a data de retorno aparece de novo em
destaque. O caminho de menor esforço para guardar essas duas lembranças é
...
o estado vai para o MODELO (o `Progresso` guarda a cerimônia pendente e a
leitura do laudo, como a gamificação faz com `celebracoes_pendentes`) e não
para a sessão.
...
def test_sem_middleware_de_sessao():
    ...
    assert not any("SessionMiddleware" in m for m in settings.MIDDLEWARE), (
        "a célula cursos não pode ter SessionMiddleware — quem assina "
...
def test_sem_app_de_sessao():
    ...
    ), "a célula cursos não pode instalar django.contrib.sessions"
...
def test_sem_backend_de_sessao_declarado_pela_celula():
    ...
    assert not hasattr(settings_da_celula, "SESSION_ENGINE"), (
        "a célula cursos não declara SESSION_ENGINE — quem assina sessão "
...
def test_o_cookie_de_csrf_tem_nome_proprio():
    ...
    `/conquistas`, as encomendas em `/encomendas` e esta célula em `/cursos`.
    Duas células gravando `csrftoken` no mesmo host é uma invalidando o
    formulário da outra.
    """
    assert settings.CSRF_COOKIE_NAME == "cursos_csrf"
