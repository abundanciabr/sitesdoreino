...
def _dependencias(dados: dict) -> list[str]:
...
def _tarefa(tarefa: str, dados: dict | None) -> dict:
    ...
    return {
        ...
        "dependencias": _dependencias(dados),
        "prova": _prova(dados),
        "substituta": str(dados.get("substitui") or ""),
    }
...
def appmax(request):
    ...
    tarefas = (
        [_tarefa(tarefa, estados.get(tarefa)) for tarefa in SEQUENCIA_APPMAX]
