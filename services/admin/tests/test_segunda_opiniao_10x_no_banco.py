"""A segunda opinião de 10x chega ao banco de produção, só para administradores.

Guarda de `0015_semear_a_segunda_opiniao_10x.py`. Mesmo desenho de
`test_alavancas_10x_no_banco.py`: no banco de teste a `0003` semeia a pasta
inteira, inclusive este documento, e a `0015` não encontraria o que inserir.
Cada teste apaga a linha antes, fabricando o banco que a migração vai
...
_semeadura = importlib.import_module(
    "apps.core.migrations.0015_semear_a_segunda_opiniao_10x"
)

NOME = "segunda-opiniao-10x-da-fabrica"
TITULO = "A segunda opinião de 10x, por cinco especialistas (setembro de 2026)"
FRASE = "A fábrica não desperdiça trabalho, desperdiça idas e voltas"
...
def _semear():
    _semeadura.semear_a_segunda_opiniao(_AppsFalso, None)
...
def test_a_segunda_opiniao_entra_no_banco_que_ja_foi_semeado(
...
@respx.mock
def test_so_quem_passa_pela_porta_le_a_segunda_opiniao(
    banco_de_producao_antes_do_documento,
):
    _semear()
