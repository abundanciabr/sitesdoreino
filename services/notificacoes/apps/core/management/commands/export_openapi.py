...
#
# Cópia do PADRÃO de `alunos`/`catalogo`/`sugestoes` (Lei 3: copia-se o padrão
...
#
# Este comando é o que torna `contracts/notificacoes.openapi.yaml` um PORTÃO em
...
def _strip_empty_component_schemas(schema: dict) -> None:
    """Esta célula declara os schemas de resposta como `ninja.Schema` tipado
    (`apps/core/api.py`), mas o contrato congelado não nomeia nenhum deles como
    componente — tudo inline nos paths, igual a `sugestoes`. django-ninja
    sempre inclui "schemas": {} em components; o contrato à mão omite a chave
    quando vazia."""
