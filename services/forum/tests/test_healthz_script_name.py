...
1. **O urlconf não pode conhecer o prefixo.** Quem o aplica é
   ...
   como `path("forum/healthz", ...)` deixa de resolver assim que o prefixo
...
2. **Qualquer isenção de middleware compara `request.path_info`, nunca
   ...
   request line que chega ao uvicorn é `GET /forum/healthz`, e nessa
   requisição `request.path` contém o prefixo em QUALQUER versão do Django.
   `request.path_info` segue `/healthz` nos dois casos.

Esta célula ainda não tem middleware próprio — a porta nasce junto com as
permissões por área. **O guarda é plantado ANTES de propósito**: quem escrever
a porta sobre `request.path` encontra este arquivo vermelho no `make ci`, em
vez de encontrar a sonda morta em produção e o container nunca ficando
`healthy`.
...
|---------------------|-------------------|------------------------------------|
| borda pública       | `/forum/healthz`  | ASGI (uvicorn atrás do Traefik)    |
...
# O prefixo público real da célula (`infra/env/forum.env`, PR 4). Escrito à mão
# aqui, e não lido de `settings`: um teste que lê a mesma variável que o código
...
def env_de_producao(settings):
    """O que o env real da VPS faz: SCRIPT_NAME=/forum."""
...
def test_urlconf_nao_conhece_o_prefixo(client):
    ...
    Sem SCRIPT_NAME configurado, `/forum/healthz` NÃO é a sonda — se este
    teste virar 200 com o JSON de saúde, alguém embutiu `/forum` numa rota e a
