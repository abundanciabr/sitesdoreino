...
| `registros/*.js` | **O livro de ocorrências.** Um arquivo pequeno por acontecimento. Só se ACRESCENTA — nunca se edita nem se apaga um registro existente. | A sessão responsável embarca o recibo da entrega pelo `make pr` e acrescenta outro registro para fatos posteriores, como publicação, verificação ou pendência. |
