...
Este projeto é construído com assistência de IAs, e a
...
- **§2 Integração automática** (8 peças; **reescrito em 13/09/2026**, e o que
  ...
  **não há revisor obrigatório, atestado, etiqueta de pouso nem encaminhamento
  por coordenação**; (3) `pouso.yml` roda código da main e integra quando
...
- **`RUNBOOK-FASE-D.md`** — manual de operação do "Esqueleto que Anda" já
  ...
  ainda é local).
- **`RUNBOOK-LOTES.md`** orienta a execução dentro do mandato: preparação,
  bancada, validação, integração automática e prestação de contas.
  As competências das fichas são por tarefa, sem papéis fixos por fornecedor.
  O protocolo antigo está preservado como histórico revogado em
  `docs/decisoes/DECISAO-triade-de-ias.md`; as leis gerais permanecem vigentes.
