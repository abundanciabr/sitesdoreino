...
> agora, então voltar a imagem não volta o esquema; e o rollback **deixou de
> ser desfeito sozinho** em 18/09/2026 (TAR-462). Até ali, a integração
> automática acordava por cron a cada 15 minutos e devolvia a célula a `:main`
> sem ninguém no circuito, porque a regra de RITOS §4 ("não mergeie nada que
> toque `infra/` com rollback ativo") nasceu quando o merge era gesto humano.
> Agora `python ci/rollback.py congelar <celula> --motivo "..."` cria uma
> referência com prazo de 6 horas no servidor, `checar_congelamento` a lê pelo
> `gh` antes de integrar, e `descongelar` a solta quando o incidente fecha.
> Falta a amarração automática: quem volta a imagem ainda roda o `congelar` à
> mão, porque ligá-lo dentro de `.github/workflows/rollback.yml` exige mandato
> do mantenedor naquele caminho. Isto mecaniza a
...
o runtime: quem revisa confere a procedência da avaliação independente.
