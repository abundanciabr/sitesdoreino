...
def test_timeout_encerra_filhos_e_netos_reais(tmp_path, pai_encerra):
    import os
    import signal
    import subprocess
    import time
...
if profundidade:
    subprocess.Popen([sys.executable, __file__, str(profundidade-1), sys.argv[2]])
...
''', encoding='utf-8')
    ...
    try:
        with pytest.raises(pr.PrazoDeValidacaoExcedido, match='TIMEOUT'):
            pr.rodar([sys.executable, 'processos.py', '2', 'sair' if pai_encerra else 'ficar'], tmp_path, log=log, prazo_segundos=2)
