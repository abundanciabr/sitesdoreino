...
def test_validacao_407_aceita_livro_ignorado_gerado_pela_muralha(tmp_path):
    ...
    provas = pr._validar(
        raiz, correto, pr.rodar,
        [[sys.executable, "-c", "from pathlib import Path; Path('painel').mkdir(); Path('painel/livro-202609.js').write_text('gerado')"]],
        lambda *_: None,
    )
...
def test_validacao_407_recusa_fonte_ignorada_fora_do_artefato_gerado(tmp_path):
    ...
    git("add", ".gitignore")
    git("commit", "-m", "ignorar fonte de teste")
    correto = git("rev-parse", "HEAD")
    git("worktree", "remove", "--force", str(raiz))
    git("worktree", "add", "--detach", str(raiz), correto)
    ...
    with pytest.raises(pr.ParouPorSeguranca, match="fontes não rastreadas"):
        pr._validar(
            raiz, correto, pr.rodar,
            [[sys.executable, "-c", "from pathlib import Path; Path('injetado.py').write_text('')"]],
            lambda *_: None,
        )
