...
#
# QUANDO ELE RODA, e a ordem não é preferência: ESTE PR vem antes do degrau 2.10
...
# nasce amarrada a um site. Esta célula não tem middleware para resolver o site
# pelo Host, e o contrato dela ainda nem congelou — o único lugar onde a
# resposta existe é o env.
#
# Ausente, a fila de uma escola inteira responderia vazia, ou pior, misturada
# com a de outro site — e nenhuma tela quebraria para avisar. Por isso o número
...
# a chave do Django e a senha do banco desta célula — as encomendas continuam
...
if [ "${BASH_SOURCE[0]:-$0}" != "$0" ]; then
  echo "PAROU POR SEGURANÇA: este arquivo foi carregado com 'source' (ou '.'), e assim um erro derrubaria a sua sessão. Rode com a palavra bash na frente: bash /tmp/e.sh"
...
# -----------------------------------------------------------------------------
# 2. TRAVA DE DERIVA — este script REESCREVE `env/encomendas.env` inteiro.
...
if [ -f env/encomendas.env ]; then
  SOBRANDO=""
  for CHAVE in $(grep -oE '^[A-Z_][A-Z0-9_]*=' "$ENV_ENCOMENDAS" | tr -d '=' | sort -u); do
  ...
  if [ -n "$SOBRANDO" ]; then
    echo "PAROU POR SEGURANÇA: o $ENV_ENCOMENDAS desta máquina tem variável que eu"
...
if psql_super -tAc "SELECT 1 FROM pg_database WHERE datname='encomendas_db'" 2>/dev/null | grep -q 1
then echo "  banco encomendas_db ... já existe"; else echo "  banco encomendas_db ... não existe"; fi
if [ -f "$ENV_ENCOMENDAS" ]
then echo "  env/encomendas.env .... já existe (guardo cópia antes de reescrever)"
else echo "  env/encomendas.env .... não existe"; fi
echo
...
QUANTOS=$(printf '%s\n' "$SITES" | grep -c . || true)
[ "${QUANTOS:-0}" -ge 1 ] || parar "o catálogo não tem NENHUM site ativo. Sem site não há a quem amarrar as encomendas, e a célula subiria sem etiqueta nenhuma. Nada foi criado."
...
if [ -n "$HOST_PEDIDO" ]; then
  ...
  echo "     bash /tmp/e.sh meshcraft.top"
...
# esvazia a fila de uma escola inteira sem quebrar tela nenhuma. Se chegamos
# aqui com o campo vazio, alguma leitura acima falhou de um jeito que eu não
# previ — e escrever o arquivo assim seria pior do que não escrever.
[ -n "${SITE_ID:-}" ] || parar "li a resposta do catálogo mas não consegui extrair o número do site. Sem SITE_ID a fila responde para o site errado e nenhuma tela quebra para avisar, então eu não escrevo o env. Nada foi criado."
echo "  site ...... ${SITE_HOST:-?}"
echo "  número .... $SITE_ID"
echo
...
T_IDENTIDADE="$(ler_de "$ENV_IDENTIDADE" TOKENS_ACEITOS_ENCOMENDAS)"
...
[ ${#T_IDENTIDADE} -ge 32 ] || parar "o token do par encomendas->identidade ficou curto demais. Nada foi alterado."

T_ALUNOS="$(ler_de "$ENV_ALUNOS" TOKENS_ACEITOS_ENCOMENDAS)"
...
psql_super -c "ALTER ROLE encomendas_user LOGIN PASSWORD '$SENHA_DB'" >/dev/null 2>&1 \
  || psql_super -c "CREATE ROLE encomendas_user LOGIN PASSWORD '$SENHA_DB'" >/dev/null \
  || parar "não consegui criar/atualizar o usuário encomendas_user."

psql_super -tAc "SELECT 1 FROM pg_database WHERE datname='encomendas_db'" 2>/dev/null | grep -q 1 \
  || psql_super -c "CREATE DATABASE encomendas_db OWNER encomendas_user" >/dev/null \
  || parar "não consegui criar o banco encomendas_db."

# A muralha de dados (Lei 2): nenhuma outra célula enxerga este banco.
psql_super -c "REVOKE ALL ON DATABASE encomendas_db FROM PUBLIC" >/dev/null \
...
umask 077
[ -f "$ENV_ENCOMENDAS" ] && cp -a "$ENV_ENCOMENDAS" "$ENV_ENCOMENDAS.bak-$(date +%s)"

# O molde é infra/env/encomendas.env.exemplo — se aquele arquivo ganhar variável
...
cat > env/encomendas.env <<ENV
DJANGO_SECRET_KEY=$CHAVE_DJANGO
DATABASE_URL=postgres://encomendas_user:$SENHA_DB@postgres:5432/encomendas_db
DEBUG=0
SCRIPT_NAME=/encomendas
SITE_ID=$SITE_ID
...
chown --reference="$ENV_REF" "$ENV_ENCOMENDAS" 2>/dev/null \
  || parar "não consegui ajustar o dono de $ENV_ENCOMENDAS — rode como root."
chmod --reference="$ENV_REF" "$ENV_ENCOMENDAS" 2>/dev/null \
  || parar "não consegui ajustar as permissões de $ENV_ENCOMENDAS — rode como root."
echo "  $ENV_ENCOMENDAS ... escrito ($(grep -c '=' "$ENV_ENCOMENDAS") variáveis)"
echo

# -----------------------------------------------------------------------------
# 6. O OUTRO LADO DOS PARES — acrescentado, nunca reescrito.
#    PROVEDOR ANTES DE CONSUMIDOR já aconteceu: o env das encomendas acima já
#    tem os tokens, mas a célula ainda não está no compose. Quando estiver, os
#    dois lados já se conhecem.
#
#    ESTE script não escreve `TOKENS_COMPLETOS_ENCOMENDAS` na identidade, e a
#    ausência continua sendo decisão: aquele degrau libera E-MAIL, e o caminho
#    da PÁGINA pede só o id opaco (`getSession`). Conceder o que não se usa é
#    superfície de graça. A lei desta célula é dura nesse ponto — nenhum dado de
#    contato do aluno chega ao cliente ([INV-ENC-S3]).
# -----------------------------------------------------------------------------
...
garantir() {  # arquivo, chave, valor, cabeçalho
  ...
  [ -f "$arq.bak-provisionar-encomendas" ] || cp -a "$arq" "$arq.bak-provisionar-encomendas"

  if grep -q "^$chave=" "$arq"; then
    sed -i "s|^$chave=.*|$chave=$valor|" "$arq" \
      || parar "a edição de $arq falhou. Há cópia intacta em $arq.bak-provisionar-encomendas."
  else
    ...
    grep -q "^# $cabecalho" "$arq" || printf '\n# %s\n# Escrito por infra/provisionar-encomendas.sh (DECISAO-fila-do-primeiro-dolar).\n' "$cabecalho" >> "$arq"
