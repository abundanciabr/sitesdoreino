...
| H20 | **A célula `identidade` está nascendo no Git e não sobe sem o que só você faz na VPS** (mesma família do H18: banco + env são seus — INV-P8, Lei 5). Sem isso o PR de infra da escada (DECISAO-celula-de-identidade §5) não pode ser mergeado: o `deploy-infra` exige todos os serviços `running`, e a célula sem banco/env entra em crashloop e derruba a verificação da plataforma inteira | UM bloco único de colar **na janela da VPS** (`deploy@srv…`), fail-closed, entregue no relatório da sessão de 25/08/2026: (1) cria o par `identidade_db`+`identidade_user` no Postgres; (2) escreve `/opt/plataforma/env/identidade.env` no molde de `infra/env/identidade.env.exemplo` — copiando as credenciais do Google de `env/sugestoes.env` (o aplicativo OAuth é o MESMO; o retorno neutro já está cadastrado desde 24/08) e a lista de staff; (3) gera no próprio servidor os tokens dos pares (`TOKENS_ACEITOS_FUNIL`, `TOKENS_ACEITOS_SUGESTOES`+`TOKENS_COMPLETOS_SUGESTOES`) e os espelha em `env/funil.env` e `env/sugestoes.env` com `>>` | 🔴 **aberto 25/08/2026 — é o degrau entre o PR 2 e o PR 3 da escada.** Sem ele os merges de `services/identidade` ficam com `deploy-celula` vermelho esperado (`armadilhas/088`) e o site segue no login antigo, funcionando |
...
| **As 8 células antigas mostram data em fuso de Chicago e em inglês.** Achado em 24/08/2026 no EVO-21: a página de avisos da `sugestoes` foi a PRIMEIRA página desta plataforma a exibir uma data ao usuário, e por isso dois padrões de fábrica do Django atravessaram sete despachos sem ninguém notar — `TIME_ZONE` nunca foi escolhido (valia `America/Chicago`, cinco horas atrás) e o formato saía em inglês (`Aug. 24, 2026, 9 a.m.`). A `sugestoes` já foi corrigida, **com guarda**; as outras oito não. Modo de falha: silencioso até a primeira data aparecer na tela, e aí é o usuário que descobre | 🟡 **declarado, não urgente** — pega carona: o próximo despacho que fizer uma célula exibir data corrige a dela de passagem, copiando o `settings` + o guarda da `sugestoes`. Só vira urgente na véspera de uma página com data ir ao ar |
