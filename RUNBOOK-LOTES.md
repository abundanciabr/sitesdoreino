# Execução de tarefas com várias partes

Use este roteiro quando o pedido tiver partes independentes. O resultado,
os alvos e as provas vêm do pedido; as leis continuam em `CLAUDE.md`,
`CONSTITUICAO.md` e `RITOS.md`. A escolha de ferramentas e competências depende
da tarefa, sem papéis fixos por fornecedor.

## Preparar

1. Ordene pelo caminho que permite comprar ou aprender. A fila e o livro são
   as fontes do trabalho pendente e dos fatos; não mantenha outro placar.
2. Delimite cada parte com célula, alvos, somente-leitura, aceite, mandato,
   modelo, esforço, receitas e armadilhas pertinentes. Use contexto direcionado;
   para aprofundamento, abra as origens recuperadas.
3. Prefira uma célula por PR, respeitando CONSTITUICAO.md, Lei 2. Mais de uma
   exige as suítes de todas as tocadas. Conte os arquivos antes de construir:
   o orçamento é 15, excluída a escrituração prevista no portão.
4. Abra a bancada pelo RITOS §1 e confira o baseline. Faça as leituras e
   medições pertinentes antes de afirmar saúde, acesso ou dependência.

## Executar

- Comece pela parte mais simples para provar o caminho até o resultado antes
  de entrar em mudanças de dinheiro. Duas tarefas na mesma célula seguem em
  série; fechamento ponta a ponta e ensaio de rollback também são seriais.

- Partes sem disputa pelos mesmos arquivos podem avançar em paralelo quando
  houver delegação autorizada; cada executor recebe brief fechado e bancada
  própria. Dependências reais seguem em série, declaradas por `Depende-de: #N`.
- Não faça refatoração de passagem, não amplie o mandato e não altere contrato
  congelado fora do RITOS §3. Segredo, dinheiro, acesso e decisões exclusivas
  do mantenedor exigem bloqueio com impacto, responsável e ação para destravar.
- Preserve alterações concorrentes. Conflito exige ler os dois lados e manter
  o trabalho alheio. Orçamento excedido exige novo recorte, nunca juntar arquivos
  artificialmente para caber.
- FAIL exige corrigir o trabalho; ERROR exige reparar ou reportar o instrumento.
  Após duas correções sem sucesso, preserve os arquivos e commits e reporte o
  diagnóstico. Uma parte bloqueada não impede concluir as independentes.
- Prove o caminho do usuário. Para dinheiro, meça o transporte e os efeitos;
  para telas, confira a renderização real. PR aberto, integração e publicação
  são estados diferentes e exigem provas próprias.

## Publicar e prestar contas

`make pr` reserva e embarca recibo, evento e metadados: não repita esses efeitos.
Revisão de código continua obrigatória como trabalho do executor; revisão
independente não é requisito para a integração automática do RITOS §2.
`pouso.yml` integra quando `muralhas` e `ci-celula-gate` passam no SHA atual,
respeitando CODEOWNERS, contrato congelado e os demais portões.

Lições, incidentes, bloqueios e publicação comprovada exigem registro novo no
livro quando não cobertos pelo recibo. Nunca edite um registro anterior.
Feche com checklist e os cinco blocos de `CLAUDE.md`, distinguindo a entrega
local do resultado completo. Se faltar algo, diga o que houve, de quem é a
próxima ação, o que destrava e o prazo; não transfira trabalho técnico rotineiro
ao mantenedor. Não anuncie execução sem executor iniciado e verificável.

## §9. Histórico dos lotes anteriores

As notas abaixo registram o contexto da época. Não são instruções vigentes;
os procedimentos atuais estão acima e nos ritos canônicos.
