...
| 2 | Mudar `contracts/` sem a label `contrato` | editar `pagamentos.openapi.yaml`, PR sem label | `muralhas` vermelho: exige label 'contrato' | ☑ [PR #36](https://github.com/abundanciabr/sitesdoreino/pull/36), fechado sem merge, 21/08/2026 |
...
| 5 | `methods/pix` importando `methods/card` | adicionar `from pagamentos.methods.card import x` | `lint-imports` vermelho no `make ci` (INV-P9) | ☑ executado em 21/08/2026 — `lint-imports` FAIL→PASS nos dois sentidos: com o import, `INV-P9 — Pix e cartao mutuamente invisiveis BROKEN` (exit 1); revertido, `KEPT` (exit 0). Lição em `services/pagamentos/LICOES.md` (commit 2e83fb8, rebase de a05e085) |
