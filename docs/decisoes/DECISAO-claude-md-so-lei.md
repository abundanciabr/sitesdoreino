# DECISÃO: o CLAUDE.md é só lei; a história mora aqui

**Decidida pelo mantenedor em 06/09/2026.** O pedido dele, com as palavras dele:

> "como Steve Jobs e/ou a Apple escreveria o arquivo CLAUDE.md para resolver
> isso definitivamente reduzindo para cerca de 30% do tamanho dele com o
> máximo de aproveitamento usando o poderoso princípio de PARETO: O CLAUDE.md
> do projeto (62.898 caracteres) é relido em toda chamada de toda sessão e de
> todo robô: 421 milhões de tokens em 4 dias. Quase metade dele é história
> (datas, número de PR, 'medido no dia', 'custou')."

## A lei, em uma frase

**O `CLAUDE.md` carrega só o que muda o que o robô faz: a regra, o comando e
quem a faz valer. O porquê (datas, números de PR, medições, o que custou) mora
neste documento, e se lê quando se precisa do motivo, não em cada chamada.**

## Por que

O `CLAUDE.md` da raiz é o único arquivo que entra sozinho no contexto de toda
sessão. Isso o torna a lei que ninguém consegue ignorar, e também o texto mais
caro do projeto: cada chamada ao modelo o reenvia inteiro.

| | antes | depois |
|---|---|---|
| tamanho | 60.005 caracteres (62.898 com quebras de linha do Windows) | 19.627 caracteres |
| quanto era história | cerca de metade | zero |
| tokens em 4 dias (medição dele) | 421 milhões | proporcional ao tamanho |

A história tinha um motivo para estar lá: a doença-mãe desta casa é lei que
depende de lembrança, e cada seção contava o dia em que a regra custou caro
para ninguém repeti-lo. O que a medição mostrou é que o remédio tinha virado
outro custo: milhares de vezes por dia, cada robô relia o dia em que o painel
colidiu, o número do PR que levou oito tentativas e a tabela dos 4,92 bilhões
de tokens. Nada disso muda o que o robô faz na chamada seguinte. Pareto: os
20% do texto que mudam comportamento (a regra, o comando, o mecanismo) ficam;
os 80% que explicam o porquê vêm para cá, onde custam zero até alguém precisar.

## O que é lei e o que é história

Fica no `CLAUDE.md`:

- a regra, no imperativo;
- o comando exato, quando há um;
- a linha `**Quem faz valer:**`, com os caminhos;
- uma referência (`armadilhas/NNN`, `docs/decisoes/...`) quando o robô
  precisa de mais.

Vem para cá:

- a data em que a lei nasceu e o que a motivou;
- números de PR, registros e medições;
- "custou", "medido no dia", "já aconteceu assim";
- as versões anteriores da regra e por que mudaram.

A régua, em uma pergunta: **"isto muda o que o robô faz na próxima chamada?"**
Se não muda, é história.

**Lei nova segue o mesmo desenho:** a regra entra no `CLAUDE.md` no formato
acima, e a história dela entra na seção "A memória de cada lei" abaixo (um
bloco novo, no fim), ou numa `DECISAO-*.md` própria, no mesmo PR.

## Quem faz valer

`ci/padrao_de_trabalho.py` reprova o `CLAUDE.md` acima de 20.000 caracteres
(`TETO_DE_CARACTERES`), e a recusa diz para onde a história vai; o teste é
`ci/tests/test_padrao_de_trabalho.py`. Subir o teto é decisão do mantenedor.
O Padrão de Trabalho (as onze regras) continua íntegro e vigiado pelo mesmo
portão, palavra por palavra: ele é texto dele, não história.

O que o teto NÃO mede, dito na cara: se um texto dentro do limite é lei ou
história. Isso é julgamento de quem escreve, com a pergunta acima.
...
*(As onze regras do Padrão continuam íntegras, palavra por palavra, na primeira seção do `CLAUDE.md`. Não se duplicam aqui.)*
...
1. **Na abertura.** Pedido que vai mudar o mundo (editar arquivo, rodar comando
   ...
   no meio do caminho.
2. **Ao fim de cada etapa.** A exigência de 05/09/2026 de mostrar o roteiro
   a cada etapa levou à repetição do plano inteiro na mesma conversa. Em
   14/09/2026, o mantenedor pediu que essa repetição fosse eliminada.
   A atualização agora é somente uma linha curta:
   `Onde estou: passo N de M. Próximo passo: ...`, sem repetir título nem checklist,
   mesmo quando uma caixa mudou ou surgiu bloqueio. "Etapa" é a conclusão
   ou o bloqueio de um passo planejado. Leituras, chamadas de ferramenta,
   mensagens automáticas e novas tentativas não são etapas. O plano completo
   aparece na abertura e no fecho; continuação da mesma tarefa não abre outro
   plano. Tarefa de um passo só tem uma etapa, e o fecho a cobre.
...
robô por algo irreparável só produz um robô travado. **A linha de progresso ao fim de
...
portão que contasse atualizações por chamada de ferramenta cobraria progresso
...
**Quem faz valer:** `ci/prestacao_de_contas.py` (o gancho `Stop` recusa o fim do turno que mudou o mundo sem os seis blocos e sem o checklist marcado; o `UserPromptSubmit` exige o plano na abertura) · `ci/tests/test_prestacao_de_contas.py`. A atualização intermediária em uma linha é orientada pelo aviso, sem portão que imponha sua obediência.
