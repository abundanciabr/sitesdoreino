...
Abra frentes conforme trabalho útil, sem teto organizacional de quantidade ou profundidade. Agentes podem delegar a outros agentes dentro do mandato, conforme CLAUDE.md. Respeite os slots do runtime, enfileire trabalho excedente e recolha resultados antes de liberar agentes pelo mecanismo suportado. Cada agente recebe objetivo, pai responsável, arquivos exclusivos, dependências, aceite e evidência. Mantenha dependências em série, sem duplicar tarefas nem criar ciclos. Cada pai consolida seus descendentes; decisões exclusivas do mantenedor sobem até a sessão que recebeu o pedido. Os especialistas devolvem sínteses com fonte, achado, limite e consequência. A [orientação oficial sobre subagentes](https://learn.chatgpt.com/docs/agent-configuration/subagents) recomenda delegação explícita de trabalho independente e síntese dos resultados.

O modelo e o esforço de cada brief vêm de `python ci/economia_da_fabrica.py brief`. No Codex, todos os agentes criados usam gpt-6-luna com esforço high, declarado nos parâmetros da ferramenta e conferido na ficha; a conversa principal preserva o modelo escolhido pelo mantenedor. Não substitua Luna por outro modelo se o runtime não o oferecer. Orçamento de contexto não é teto de agentes. Use as ferramentas que existem no ambiente. Não crie tarefas separadas do Codex, conversas novas ou automações agendadas sem pedido explícito. A equipe trabalha nesta conversa. A memória persistente está nos artefatos versionados e nos registros, não na sobrevivência dos agentes. Não infira autorização para agendamento, assinatura, crédito ou gasto externo.
