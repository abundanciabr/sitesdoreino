...
### Administração no site da VPS

**ATUALIZAÇÃO DE 22/09/2026.** A direção anterior de usar o painel no localhost foi revogada. A administração é acessada no site da VPS, em `/admin/`, e seus documentos em `/admin/documentos/`; não indicar `ci/ligar_administracao.py` como caminho de acesso. O PR #1921 integrou no commit `4a171b12ad10f552a3ea45c189ed47c4703fb7a8`. Medição após a integração: `/` respondeu HTTP 200, `/admin/` e `/admin/documentos/` responderam HTTP 302 para autenticação. Isso comprova que as rotas estão publicadas e protegidas; não comprova o login do mantenedor nem a abertura de um documento. As referências a TAR-419, TAR-422 e TAR-433 abaixo são histórico de uma arquitetura cancelada, não instruções vigentes.
...
| HIPÓTESE: titulares sem substituto fragilizam continuidade do acesso administrativo | Alto se recurso indisponível: concentração em pessoa | [responsabilidades](../../../painel/responsabilidades.json), autenticação e rota `/admin/` no site. Histórico não prova saúde; exercitar retomada autorizada |
...
| FATO: nomenclaturas/funções históricas coexistem | Baixo isoladamente, Médio se viram instrução | Tríade, maestro, despacho, pouso e GPS exigem revisão/fonte. O painel local foi cancelado; não orientar seu uso |
...
| 3. TAR-422, leitura de saúde e métricas no localhost | Cancelada pela decisão registrada em [20260920-046](../../../painel/registros/20260920-046-o-resto-do-admin-no-localhost-foi-cancelado.js); não retomar como caminho de acesso ao admin | Nenhuma execução local deve ser iniciada; a administração permanece no site da VPS | Cancelada; referência histórica preservada |
