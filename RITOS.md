...
2. O executor publica o PR pronto com a validação da mudança. Não há revisor
   obrigatório, atestado, etiqueta de pouso ou encaminhamento pela coordenação.
...
2. **Trabalho novo que um despacho descobre vira tarefa registrada**
   (`python ci/fila.py criar ...`, número do almoxarife), e volta à sessão responsável,
   que confere o mandato: nunca item de memória de sessão, nunca lista
...
3. **Concluir exige evidência** (`concluir --evidencia <URL>`) — sem prova o
   ...
   **Adiamento não existe como estado.** Nenhum robô pode deixar uma tarefa
   deliberadamente para depois sem anuência expressa do mantenedor. O comando
   e o painel devem mostrar o impedimento na hora, com `espera: mantenedor` ou
   `espera: fila`; a tarefa continua aberta até um evento real de devolução,
   conclusão ou cancelamento autorizado.
